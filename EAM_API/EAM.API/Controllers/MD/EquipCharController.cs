...
namespace EAM.API.Controllers.MD
{
    ...
    [ApiController]
    public class EquipCharController(IEquipCharService service) : ControllerBase
    {
        public readonly IEquipCharService _service = service;
        ...
        [HttpGet("GetDetail/{equnr}")]
        public async Task<IActionResult> GetDetail([FromRoute] string equnr)
        {
            var transferObject = new TransferObject();
            var result = await _service.GetDetail(equnr);
            if (_service.Status)
            {
                transferObject.Data = result;
